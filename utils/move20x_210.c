...
char dirname[80],name[USER_NAME_LEN+4],new_filename[80],old_filename[80],cmd[80];
DIR *dirp;
struct dirent *dp;
...
for (level=JAILED;level<=GOD;level++) {
  ...
  while((dp=readdir(dirp))!=NULL) {
    ...
    if (strstr(dp->d_name,".D")) {
      ...
      sprintf(old_filename,"%s/%s",dirname,dp->d_name);
      sprintf(new_filename,"%s/%s/%s",USERFILES,USERCOMMANDS,dp->d_name);
      cnt_c++;
      }
    else if (strstr(dp->d_name,".H")) {
      sprintf(old_filename,"%s/%s",dirname,dp->d_name);
      sprintf(new_filename,"%s/%s/%s",USERFILES,USERHISTORYS,dp->d_name);
      cnt_h++;
      }
    else if (strstr(dp->d_name,".P")) {
      sprintf(old_filename,"%s/%s",dirname,dp->d_name);
      sprintf(new_filename,"%s/%s/%s",USERFILES,USERPROFILES,dp->d_name);
      cnt_p++;
      }
